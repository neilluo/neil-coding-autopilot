...
# ============================================================
# Closes the redirection bypass of guard-controller-write.sh. That gate only
# covers file-writing TOOLS (Write/Edit/…). A controller could still write
# source during a run via the shell:  `cat > x.py`, `echo .. >> x.go`,
# `… | tee x.java`, `dd of=x.rs`. This hook gates the Bash tool for exactly
# that case and denies it (exit 2), while leaving all legitimate orchestration
# shell usage untouched.
#
...
#   3. no source-write  no redirect/tee/dd to a SOURCE path     => allow
#   4. otherwise        controller shell-writing source at run  => DENY (exit 2)
#
# Deliberately conservative: only a redirection/tee/dd whose TARGET has a known
# source-code extension AND is not whitelisted (*.md / autopilot/ / .qoder/ /
# /dev/*) triggers a deny. Extensionless/.txt/quoted-in-code writes fail-open
# (documented residual) — the point is to shut the obvious `cat > x.py` hole
# without ever locking up a controller's git/verify/dispatch shell commands.
# ============================================================
...
# Gate only the shell/terminal tool (canonical + IDE aliases). Else allow.
case "$TOOL_NAME" in
...
# --- layer 3: extract write-redirection targets from the command ---
# Match targets after `>` / `>>` / `tee [-a]` / `dd of=`. BSD/macOS-safe grep
# (no -P). Anything we cannot classify as a source write => fail-open (allow).
[ -z "$CMD" ] && exit 0
_targets="$(printf '%s' "$CMD" \
  | grep -oE '(>>?[[:space:]]*|[[:space:]]tee[[:space:]]+(-a[[:space:]]+)?|[[:space:]]dd[[:space:]]+[^|;&]*of=)["'"'"']?[^ "'"'"'|;&<>()]+' 2>/dev/null \
  | sed -E 's/^[[:space:]]*(>>?|tee([[:space:]]+-a)?|dd[[:space:]].*of=)[[:space:]]*//; s/^["'"'"']//' \
  || true)"

[ -z "$_targets" ] && exit 0   # no redirect/tee/dd target => allow

# --- layer 4: deny if any target is a SOURCE file not on the whitelist ---
_deny_path=""
_IFS_SAVE="$IFS"; IFS='
'
...
REASON="控制器禁止用 shell 重定向写源码(${_deny_path})；autopilot 开发一律经 run-track-a.sh 托管 fresh qodercli worker（结构性硬约束，非仅约定）。"
