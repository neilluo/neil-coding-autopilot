...
- 静态审阅与复盘报告都可能漏掉 runtime-only 的 bug（本项目已撞到两例，均在**真跑 Track A** 时才现形：① `timeout` on macOS → dispatch.sh exit 127；② `grep -P` on macOS BSD grep → parse-status.sh exit 2，且旧正则对 `**Status:** DONE` 会误取到 `**`。读代码/读文档都看不出，一跑就现形）。

## shell 可移植性（macOS/BSD vs GNU）

- 别假设 GNU 工具存在：`timeout`（mac 为 `gtimeout`，需 coreutils）、`sed -i`（BSD 需 `-i ''`）、`date`、`readlink -f`、`grep -P` 等行为不同或缺失。
